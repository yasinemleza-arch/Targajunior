...
export default function FloatingWhatsApp({ lang }: FloatingWAProps) {
  ...
  return (
    <div className={`fixed bottom-6 z-50 ${lang === 'ar' ? 'left-6' : 'right-6'} flex flex-col items-end gap-3`}>
      ...
      {open && (
        <div
          className="bg-white rounded-xl shadow-2xl p-4 w-64 border border-neutral-100 animate-fadeInUp"
          dir={lang === 'ar' ? 'rtl' : 'ltr'}
        >
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="font-semibold text-xs text-neutral-900">Targa Junior</p>
                <div className="flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#25d366]" />
                  <span className="text-[10px] text-neutral-500">
                    {lang === 'ar' ? 'متاح الآن' : lang === 'en' ? 'Available' : 'Disponible'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-[#25d366] text-white text-xs font-semibold hover:bg-[#1db954] transition-colors"
          >
      ...
      <button
        ...
        className="w-14 h-14 rounded-full bg-[#25d366] text-white flex items-center justify-center shadow-xl hover:bg-[#1db954] transition-all duration-300 hover:scale-105"
        aria-label="WhatsApp"
      >
        {open ? (
          <X size={22} />
